...
class SnapshotRenderer {
  ...
  render() {
    ...
    html = prefix + ['<style>*,*::before,*::after { visibility: hidden }</style>', `<style>*[__playwright_target__="${this.snapshotName}"] { background-color: #6fa8dc7f; }</style>`, `<script>${snapshotScript()}</script>`].join('') + html;
    return {
      html,
      pageId: snapshot.pageId,
      frameId: snapshot.frameId,
