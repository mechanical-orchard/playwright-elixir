...
async function launchDockerGridAgent(agentId, gridURL) {
  var _process$env$PWTEST_I, _info$NetworkSettings;
  ...
  const imageName = (_process$env$PWTEST_I = process.env.PWTEST_IMAGE_NAME) !== null && _process$env$PWTEST_I !== void 0 ? _process$env$PWTEST_I : `mcr.microsoft.com/playwright:v${require('../../package.json').version}-focal`;
  const pwImage = images.find(image => {
    var _image$RepoTags;

    return (_image$RepoTags = image.RepoTags) === null || _image$RepoTags === void 0 ? void 0 : _image$RepoTags.includes(imageName);
  });
