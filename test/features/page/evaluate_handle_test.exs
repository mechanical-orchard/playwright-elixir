defmodule Test.Features.Page.EvaluateHandleTest do
  ...
  describe "Page.evaluate_handle/2" do
    ...
    test "returns a handle that can be used as a later argument, as a handle to an object", %{page: page} do
      ...
      result = Page.evaluate(page, "function(h) { return h.userAgent; }", handle)
    ...
    test "returns a handle that can be used as a later argument, as a handle to a primitive type", %{page: page} do
      ...
      result = Page.evaluate(page, "function(h) { return Object.is(h, 42); }", handle)
      assert result === true

      result = Page.evaluate(page, "function(n) { return n; }", handle)
      assert result === 42
    end
    ...
    test "works with a handle that references an object", %{page: page} do
      ...
      result = Page.evaluate(page, "function(o) { return o; }", handle)
